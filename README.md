...
After compression methods are selected, the model <i>must be retrained</i> using fixed-point weight values, alternative math, and other algorithm changes due to compression, in order to reach (i) a tolerable loss in accuracy (typically 2 - 5%) and (ii) required real-time performance (typically some number of frames per sec).  The process of training, testing, compression, retraining, and more testing, may require many iterations until acceptable tradeoffs are reached.  This is a time-consuming, iterative process and must be done on high performance cloud or private servers and not on the embedded target, where it would take prohibitive amounts of time.
...
As a potential basis for a c66x device-accurate deep learning cloud solution, the architecture shown in the pictures and links below already works.  Currently this architecture supports OpenCV, C/C++, Python, c66x math and DSP libraries, 8-bit MAC operations (32-bit accumulation), and other essential deep learning components, but a substantial amount of additional work is needed to create a deep learning CICD reference solution.  At minimum, such a solution should incorporate TensorFlow, Caffe, the deep learning models mentioned in the Readme intro above, and the SqueezeNet and MobileNet compression models.
